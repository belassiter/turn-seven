...
function main() {
  ...
  const prettierArgs = usePnpm
    ? ['exec', 'prettier', '--write', ...targetFiles]
    : ['prettier', '--write', ...targetFiles];

  if (!run(prettierCmd, prettierArgs)) {
    console.error(
      '[verify:ai] prettier failed. This usually means the project devDependencies (prettier) are not installed. Run `pnpm install` and try again.'
    );
    process.exit(10);
  }
  ...
  const eslintArgs = usePnpm
    ? ['exec', 'eslint', ...eslintArgsBase]
    : ['eslint', ...eslintArgsBase];
  ...
  const eslintCheckArgs = usePnpm
    ? ['exec', 'eslint', ...eslintCheckArgsBase]
    : ['eslint', ...eslintCheckArgsBase];
  ...
  const hasProblems =
    ...
    problems.some((p) => Array.isArray(p.messages) && p.messages.length > 0);
  if (hasProblems) {
  ...
  if (
    !run(
      usePnpm ? 'pnpm' : 'npx',
      usePnpm ? ['test', '--reporter', 'verbose'] : ['vitest', '--run', '--reporter', 'verbose']
    )
  ) {
