...
describe('Mid-round reshuffle + large-player deal edge cases', () => {
  it('reshuffle that introduces an action card should resolve that action and pause dealing appropriately', () => {
    ...
    const state: any = {
      ...
      discardPile: [
        ...
        { id: 'd0', suit: 'number', rank: '8', isFaceUp: true },
  ...
  it('startNextRound can deal to a very large player count using deck+discard and not lose cards', () => {
    ...
    const players: any[] = [];
    for (let i = 0; i < playerCount; i++) players.push({ id: `p${i}`, name: `P${i}`, hand: [], hasStayed: false, isActive: true, hasBusted: false, pendingImmediateActionIds: [] });
    ...
    const restoreCreate = vi.spyOn(logic as any, 'createDeck').mockReturnValue(
      Array.from({ length: playerCount + 4 }, (_, i) => ({ id: `cd${i}`, suit: 'number', rank: String((i % 12) + 1), isFaceUp: false })).reverse()
    );
